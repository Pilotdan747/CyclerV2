...
void lambert_multi(vec R_1, vec R_2, double tof, int m, double mu, vector V[2]) {
    vector R1; R1.x = R_1(0); R1.y = R_1(1); R1.z = R_1(2);
    vector R2; R2.x = R_2(0); R2.y = R_2(1); R2.z = R_2(2);
    ...
    if (exitFlag != 1) {
        vector temp; temp.x = NULL; temp.y = NULL; temp.z = NULL;
        V[0] = temp;
        V[1] = temp;
    }
}
    
int mlambert(vector R1, vector R2, double tf, int m, double mu, vector Vel[2]) {
    int exitFlag = 0;
...
int lambert_LancasterBlanchard(vector R1, vector R2, double tf, int m, double muC, vector V[2]) {
    int exitFlag = 0;
