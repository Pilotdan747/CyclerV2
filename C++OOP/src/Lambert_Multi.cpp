...
void lambert_multi(vec R1, vec R2, double tof, int m, double mu, vec V[2]) {
    ...
    int exitFlag = mlambert(R1, R2, tf, m, mu, V);
    if (exitFlag != 1) {
        // Means algorithm failed, give back junk data
        vec temp = {1e9, 1e9, 1e9};
        V[0] = temp;
        V[1] = temp;
    }
}
    
int mlambert(vec R1, vec R2, double tf, int m, double mu, vec Vel[2]) {
    int exitFlag = 0;
    ...
    vec crossprd = cross(r1, r2);
    double mcr = norm(crossprd);
    vec nrmunit = crossprd/mcr;
    ...
    if (bad) {
        exitFlag = lambert_LancasterBlanchard(R1, R2, longway*tf*T, leftbranch*m, mu, Vel);
        return exitFlag;
    }
    ...
    vec ih =  longway*nrmunit;
    vec r2n = r2/mr2vec;

    vec crsprd1 = cross(ih, r1);
    vec crsprd2 = cross(ih, r2n);
    ...
    vec V1 = (Vr1*r1 + Vt1*crsprd1)*V; 
    vec V2 = (Vr2*r2n + Vt2*crsprd2)*V;
...
int lambert_LancasterBlanchard(vec R1, vec R2, double tf, int m, double muC, vec V[2]) {
    ...
    vec r1unit = R1/r1;
    vec r2unit = R2/r2;
    vec crsprd = cross(R1, R2);
    double mcrsprd = norm(crsprd);
    vec temp = crsprd/mcrsprd;
    vec th1unit = cross(temp, r1unit);
    vec th2unit = cross(temp, r2unit);
    ...
    vec V1, V2;
    V1 = Vr1*r1unit + Vtan1*th1unit;
    V2 = Vr2*r2unit + Vtan2*th2unit;
